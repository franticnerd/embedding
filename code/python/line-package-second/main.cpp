...
char node_file_w[MAX_STRING]="node-w.txt", node_file_l[MAX_STRING]="node-l.txt", node_file_t[MAX_STRING]="node-t.txt";  
char link_file_ww[MAX_STRING]="edge-ww.txt",link_file_ll[MAX_STRING]="edge-ll.txt",link_file_tt[MAX_STRING]="edge-tt.txt";
char link_file_wl[MAX_STRING]="edge-wl.txt",link_file_lt[MAX_STRING]="edge-lt.txt",link_file_tw[MAX_STRING]="edge-tw.txt";
...
line_node node_w, node_l, node_t;
line_hin hin_ww, hin_ll, hin_tt, hin_wl, hin_lt, hin_tw, hin_lw, hin_tl, hin_wt;
line_trainer_edge trainer_edge_ww, trainer_edge_ll, trainer_edge_tt, trainer_edge_wl, trainer_edge_lt, trainer_edge_tw, trainer_edge_lw, trainer_edge_tl, trainer_edge_wt;
...
void TrainModel() {
    ...
    node_w.init(node_file_w, vector_size);
    node_l.init(node_file_l, vector_size);
    node_t.init(node_file_t, vector_size);

    hin_ww.init(link_file_ww, &node_w, &node_w);
    hin_ll.init(link_file_ll, &node_l, &node_l);
    hin_tt.init(link_file_tt, &node_t, &node_t);
    hin_wl.init(link_file_wl, &node_w, &node_l);
    hin_lt.init(link_file_lt, &node_l, &node_t);
    hin_tw.init(link_file_tw, &node_t, &node_w);
    hin_lw.init(link_file_lw, &node_l, &node_w);
    hin_tl.init(link_file_tl, &node_t, &node_l);
    hin_wt.init(link_file_wt, &node_w, &node_t);
    ...
    node_w.output(output_file_w, binary, 0);
    node_l.output(output_file_l, binary, 0);
    node_t.output(output_file_t, binary, 0);
    node_w.output(context_file_w, binary, 1);
    node_l.output(context_file_l, binary, 1);
    node_t.output(context_file_t, binary, 1);
}
